...
CREATE TABLE `amf_message_forwarding` (
  ...
  `callback_required` tinyint unsigned NOT NULL DEFAULT 0 COMMENT '是否需要回调，1-是，0-否',
  `callback_url` varchar(200)  DEFAULT NULL COMMENT '消息发送结果回调地址',
  `forwarding_status` tinyint unsigned NOT NULL COMMENT '转发处理状态：0-待转发，1-转发中，2-已完成',
  ...
  `callback_status` tinyint unsigned DEFAULT NULL COMMENT '回调处理状态：0-待回调，1-回调中，2-已完成',
  `callback_result` tinyint unsigned DEFAULT NULL COMMENT '回调结果：0-失败，1-成功',
  `callback_success_time` timestamp NULL DEFAULT NULL COMMENT '回调成功时间',
...
CREATE TABLE `amf_message_forwarding_history` (
  ...
  `callback_required` tinyint unsigned NOT NULL DEFAULT 0 COMMENT '是否需要回调，1-是，0-否',
  `callback_url` varchar(200)  DEFAULT NULL COMMENT '消息发送结果回调地址',
  `forwarding_status` tinyint unsigned NOT NULL COMMENT '转发处理状态：0-初始化，1-处理中，2-完成',
  `forwarding_result` tinyint unsigned NULL DEFAULT NULL COMMENT '转发结果：0-失败，1-成功',
  `forwarding_success_time` timestamp NULL DEFAULT NULL COMMENT '转发成功时间',
  `forwarding_retry_times` tinyint NOT NULL DEFAULT 0 COMMENT '转发重试次数',
  `callback_status` tinyint unsigned DEFAULT NULL COMMENT '回调处理状态：0-初始化，1-处理中，2-完成',
  `callback_result` tinyint unsigned DEFAULT NULL COMMENT '回调结果：0-失败，1-成功',
  `callback_success_time` timestamp NULL DEFAULT NULL COMMENT '回调成功时间',
  `callback_retry_times` tinyint DEFAULT NULL COMMENT '回调重试次数',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间，默认值为系统当前时间',
