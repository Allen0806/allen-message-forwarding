...
import static com.allen.message.forwarding.constant.StatusCodeConstant.MF_1001;
import static com.allen.message.forwarding.constant.StatusCodeConstant.MF_1002;
	...
	public void receive(MessageReceiveDTO messageReceiveDTO) {
		...
		if (messageConfig == null || messageConfig.getForwardingConfigs() == null
			...
			LOGGER.error("根据给定的消息ID未获取到对应的消息配置信息或消息配置信息有误，消息流水号：{}，消息ID：{}", messageReceiveDTO.getMessageNo(),
					messageId);
			throw new CustomBusinessException(MF_1001.getCode(), MF_1001.getMessage());
		}
		if (!messageConfig.getBusinessLineId().equals(messageReceiveDTO.getBusinessLineId())
			...
			throw new CustomBusinessException(MF_1002.getCode(), MF_1002.getMessage());
		}
